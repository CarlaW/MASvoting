...
public class TVA {
			...
			if (newHapp[voterID - 1] > oldHapp) {
				if (trueFavorite == newOutcome[0].option) {
					reason = "Using " + method + ", voter # " + voterID
							+ " preferred the new outcome because the true favourite candidate "
							+ Character.toString(trueFavorite) + " went up to the first place.";
				} else {
					reason = "Using " + method + ", voter # " + voterID
							+ " preferred the new outcome because another preferred candidate "
							+ Character.toString(newOutcome[0].option) + " went up to the first place.";
				}
			} else {
				reason = "Using " + method + ", voter # " + voterID
						+ " did not get more happy, but his true favourite candidate "
						+ Character.toString(trueFavorite) + " went up at least one position.";
			}
		}
		return reason;
	}
			...
			for (int j = 0; j < i; j++) {
				...
				if (shouldManipulate(truePreference[0], oldOutcome, newOutcome, oldHappiness, newHappiness[voterID])) {
					String reasoning = reasoningString(truePreference[0], oldOutcome, newOutcome, oldHappiness,
							newHappiness, voterID, "compromising");
					setOfOptions.add(
	...
	public ArrayList<StrategicVotingOption> tryBulletVoting(int voterID) {
		...
		for (int i = 0; i < numOfCandidates; i++) {
			...
			if (shouldManipulate(truePreference[0], oldOutcome, newOutcome, oldHappiness, newHappiness[voterID])) {
				String reasoning = reasoningString(truePreference[0], oldOutcome, newOutcome, oldHappiness,
						newHappiness, voterID, "bullet voting");
				setOfOptions
	...
	private void displayInConsole(int votingScheme) {
		...
		/*
		 * format = "%-25s%25s\n"; System.out.println(); for (int i=0; i<result.size();
		 * i++){ for (int j=0; j<result.get(i).size(); j++){ System.out.println();
		 * StrategicVotingOption s = result.get(i).get(j);
		 * System.out.println("Strategic voting option # " + (i+1) + " for voter # " +
		 * (s.voterID+1) + ":"); System.out.format(format, "v", "newO");
		 * System.out.println("---------------------------------------------------");
		 * for (int k=0; k<s.v.length; k++) { System.out.format(format,
		 * String.valueOf(s.v[k]), String.valueOf(s.newO[k].option) + ": " +
		 * Integer.toString(s.newO[k].count)); }
		 * System.out.println("---------------------------------------------------");
